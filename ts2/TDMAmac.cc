...
void TDMAmac::handleUpperMsg(cMessage* msg){
    ...
    MyMacPkt *mac = static_cast<MyMacPkt *>(encapsMsg(static_cast<cPacket*>(msg)));
...
void TDMAmac::handleSelfMsg(cMessage* msg)
{
      switch (msg->getKind())
      {
          ...
          case 0:
          {
              ...
              MyMacPkt* data = macPktQueue.front()->dup();
              data->setKind(0);
              attachSignal(data);
 ...
 */
TDMAmac::macpkt_ptr_t MyTDMAmac::encapsMsg(cPacket* msg) {

    MyMacPkt *pkt = new MyMacPkt(msg->getName(), msg->getKind());
...
void TDMAmac::handleLowerMsg(cMessage* msg) {
    MyMacPkt *const mac  = static_cast<MyMacPkt *>(msg);
