...
void Master::initialize()
{
    ev<<"DXw: Initialize PtpMaaster"<<endl;
        ...
        EV << "Core broadcasts REGISTER packet!" << endl;
...
void Master::handleMessage(cMessage* msg)
{
    ...
    else if(whichGate == lowerGateIn)
    {
        ...
        {
            EV<<"This ia a PtpPkt packet, Master now is processing it "<<endl;
...
void Master::handleSelfMessage(cMessage *msg){
    ...
    pck->setTsTx(SIMTIME_DBL(simTime())); // set transmission timestamp ts1 on SYNC
...
void Master::handleSlaveMessage(PtpPkt *msg){

    switch (msg -> getPtpType()){
        case SYNC:
        {
                ev<<"Master::handleSlaveMessage() receives a SYNC packet, ignore.\n";
                break;
        }
        case DRES:
        {
        ...
        case REG:
        {
            // Receive Register packet from other nodes and no reply
